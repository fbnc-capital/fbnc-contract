...
contract FbncPreSale is Ownable {
    using SafeMath for uint256;
    ...
    event PreSale(address claimer, uint256 amount);
    ...
    function preSale(uint256 _amount) public {
        require(_amount >= 1e18, "must > 1");
        IBEP20(tokenPayment).safeTransferFrom(msg.sender, hub, _amount);
        uint256 totalToken = _amount.div(pricePerToken).mul(1000);
        _sale(totalToken);
    }

    function _sale(uint256 totalToken) internal {

        IBEP20(tokenReward).safeTransfer(msg.sender, totalToken);
        emit PreSale(msg.sender, totalToken);
    }
